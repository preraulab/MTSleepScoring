...
%%%%%%%%%%%%%%%% public properties %%%%%%%%%%%%%%%%%%
    ...
    methods (Access = private)
        ...
        function init_fig(obj)
            ...
            %Make Labels
            uicontrol(obj.mainfig_h,'units','normalized','Style','text','string','Clim Min','Position',[0.8737 0.9718 0.0219 0.0163],'backgroundcolor',get(obj.mainfig_h,'color'),'horizontalalign','right');
            uicontrol(obj.mainfig_h,'units','normalized','Style','text','string','Clim Max','Position', [0.8602 0.9479 0.0352 0.0158],'backgroundcolor',get(obj.mainfig_h,'color'),'horizontalalign','right');
            ...
            %             %                   CREATE FIGURE MENUS
            %             %---------------------------------------------------------
            % %             set(obj.mainfig_h,'MenuBar','None')
            %
            %             %Make menu item to put in fixed time scale
            %             m=uimenu('Label','Time Window');
            %             %Change the time scale
            %             uimenu(m,'Label','Set Window Zoom...','callback',@set_timescale,'accelerator','z');
            %
            %             %Make menu item to put in fixed time scale
            %             m=uimenu('Label','Scoring');
            %             %Change the time scale
            %             uimenu(m,'Label','Save Scoring...','callback',@(src,evnt)em.save,'accelerator','s');
            %             uimenu(m,'Label','Load Scoring...','callback',@(src,evnt)em.load,'accelerator','s');
            %
            
            %Delete excess menus
            %             allhandles = findall(obj.mainfig_h);
            %             menuhandles = findobj(allhandles,'type','uimenu');
            %             mergeidx = find(strcmpi({menuhandles.Text},'Merge Axes'));
            %             for ii=1:length(mergeidx)
            %                 try
            %                     delete(menuhandles(mergeidx(ii)));
            %                 catch
            %                 end
            %             end
            %
        ...
        function update_hypno(obj, event_time)
            ...
            if ~isempty(event_time)
                etimes(obj.event_marker.selected_ind) = event_time;
            end
            
            %Sort the times and plot the hypnogram
            [setimes,sind]=sort(etimes);
            estage=estage(sind);
            
            %Stairs is SLOW
            x=[obj.stimes{obj.curr_resolution}(1) setimes obj.stimes{obj.curr_resolution}(end)];
            y=[0 estage(:)' 0];
